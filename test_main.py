    ...
    def is_market_open(self):
        ...
        try:
                ...
                if self.morning_update[0] == self.current_hour:
                    if is_open:
                        ...
                        if not self.dry_run:
                            ...
                            print(f"[DRY RUN] Would tweet: Today {current_date} the {self.exchange} is open from {opening_hour} to {closing_hour}.")
                    else:
                        ...
                        if not self.dry_run:
                            ...
                            print(f"[DRY RUN] Would tweet: Today {current_date} the {self.exchange} is closed.")
            ...
            if not self.dry_run:
                ...
                print(f"[DRY RUN] Would send telegram: ⚠️ API Error: Failed to fetch market data - {e}")
            ...
            if not self.dry_run:
                ...
                print(f"[DRY RUN] Would send telegram: ⚠️ Error parsing market data - {e}")
            self.market_open = True
